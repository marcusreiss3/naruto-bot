...
export async function openSheetChannel(guild: Guild, member: GuildMember): Promise<OpenSheetResult> {
  ...
  if (existing) {
    ...
    if (existing.stage === "CLAN_CHOICE" && !existing.clanChoiceExpiresAt) {
      existing = await prisma.sheetCreationSession.update({
        where: { id: existing.id },
        data: { clanChoiceExpiresAt: new Date(existing.createdAt.getTime() + CLAN_CHOICE_TTL_MS) },
      });
    }
    await resolveExpiredClanChoice(guild.client, existing, new Date());
    existing = await prisma.sheetCreationSession.findUnique({ where: { id: existing.id } });
  }
  ...
  try {
    session = await prisma.sheetCreationSession.create({
      data: {
        ...
        channelExpiresAt: new Date(Date.now() + SHEET_CHANNEL_TTL_MS),
        clanChoiceExpiresAt: new Date(Date.now() + CLAN_CHOICE_TTL_MS),
      },
    });
  } catch (error) {
  ...
  await runClanAnimation(channel, session.id, clanOptions, session.clanChoiceExpiresAt);
  await armSheetScheduler(guild.client);
...
async function runClanAnimation(
  ...
  await message.edit({
    ...v2Edit(card([
      ...
      text(choiceDeadline
        ? `-# Escolha até <t:${Math.floor(choiceDeadline.getTime() / 1_000)}:F> (<t:${Math.floor(choiceDeadline.getTime() / 1_000)}:R>). Depois disso, uma destas opções será escolhida aleatoriamente.`
        : "-# Se o prazo de escolha já tiver acabado, o bot selecionará aleatoriamente uma destas opções."),
      divider(),
      ...sections,
      divider(),
...
async function promptName(channel: TextChannel, data: SheetData) {
  ...
  await channel.send(v2Public(card([
    ...
    text("Escreva agora somente o nome próprio do seu personagem."),
    divider(),
    text(`**Não inclua o sobrenome do clã.**\nDigite, por exemplo, \`Marcus\`, e a ficha registrará \`Marcus ${clan.name}\`.`),
    text("-# São aceitas apenas letras, espaços, hífen e apóstrofo; de 2 a 24 caracteres."),
  ])));
}

async function resolveExpiredClanChoice(
  client: Client,
  session: PersistedSession,
  now: Date,
): Promise<boolean> {
  if (
    session.stage !== "CLAN_CHOICE" ||
    !session.clanChoiceExpiresAt ||
    session.clanChoiceExpiresAt.getTime() > now.getTime()
  ) return false;

  const data = parseData(session.dataJson);
  if (!data.clanOptions.length) {
    log.error(`Ficha ${session.id} não possui opções de clã salvas.`);
    return false;
  }
  const option = randomClanOption(data.clanOptions);
  ...
  if (channel) {
    const clan = CLAN_INDEX.get(option.clanId)!;
    await channel.send(v2Public(card([
      heading("Prazo de escolha encerrado"),
      text("Como nenhuma opção foi escolhida em dez minutos, o bot definiu aleatoriamente uma das três origens que já estavam salvas."),
      divider(),
      text(`**${VILLAGE_NAMES[option.villageId]} — Clã ${clan.name}**`),
    ], "aviso"))).catch((error) => log.error(`Falha ao avisar escolha automática da ficha ${session.id}:`, error));
    await promptName(channel, data).catch((error) => log.error(`Falha ao continuar ficha ${session.id}:`, error));
  }
  return true;
}
...
function ageInstruction(data: SheetData) {
  ...
  return text(
    `## Idade do personagem\nDigite somente um número inteiro entre **${minimum} e 13**.\n` +
      `-# ${prodigy ? "O traço Prodígio Ninja libera as idades 9 e 10." : "Outros traços começam a partir dos 11 anos."}`,
  );
}
...
async function promptAppearance(channel: TextChannel) {
  await channel.send(v2Public(card([
    ...
    text("Envie uma imagem anexada ou cole um link direto de imagem aceito pelo Discord."),
    divider(),
...
async function resumeSheet(channel: TextChannel, session: PersistedSession): Promise<void> {
  ...
  switch (session.stage as Stage) {
    case "CLAN_CHOICE":
      await runClanAnimation(channel, session.id, data.clanOptions, session.clanChoiceExpiresAt);
      return;
    case "NAME":
...
export async function handleSheetMessage(message: Message): Promise<boolean> {
  ...
  if (session.stage === "APPEARANCE") {
    ...
    data.appearanceUrl = imageUrl;
    if (!await transitionSession(session.id, "APPEARANCE", "CONFIRM", data)) return true;
    await sendAppearancePreview(channel, session.id, data);
    return true;
  }
...
export async function handleSheetButton(interaction: ButtonInteraction): Promise<void> {
  ...
  if (action === "clan") {
    ...
    if (session.clanChoiceExpiresAt && session.clanChoiceExpiresAt.getTime() <= Date.now()) {
      await resolveExpiredClanChoice(interaction.client, session, new Date());
      await interaction.reply({
        content: "Os dez minutos terminaram e uma das opções salvas foi escolhida aleatoriamente. Continue pelo painel mais recente.",
    ...
    const changed = await prisma.sheetCreationSession.updateMany({
      where: {
        ...
        OR: [
          { clanChoiceExpiresAt: null },
          { clanChoiceExpiresAt: { gt: new Date() } },
        ],
      },
      data: {
        stage: "NAME",
  ...
  const sessions = await prisma.sheetCreationSession.findMany({
    where: {
      OR: [
        ...
        { stage: "CLAN_CHOICE", clanChoiceExpiresAt: null },
      ],
    },
  });
    ...
    await prisma.sheetCreationSession.update({
      ...
      data: {
        ...
        ...(session.stage === "CLAN_CHOICE" && !session.clanChoiceExpiresAt
          ? { clanChoiceExpiresAt: new Date(session.createdAt.getTime() + CLAN_CHOICE_TTL_MS) }
          : {}),
      },
    });
  }
...
async function processDueSheetEvents(client: Client): Promise<number> {
  ...
  try {
    const now = new Date();
    const expiredChoices = await prisma.sheetCreationSession.findMany({
      where: {
        stage: "CLAN_CHOICE",
        clanChoiceExpiresAt: { lte: now },
      },
      orderBy: { clanChoiceExpiresAt: "asc" },
    });
    let processed = 0;
    for (const session of expiredChoices) {
      if (await resolveExpiredClanChoice(client, session, now)) processed += 1;
    }
...
export async function armSheetScheduler(client: Client): Promise<void> {
  ...
  scheduler = null;
  const [nextChannel, nextChoice] = await Promise.all([
    prisma.sheetCreationSession.findFirst({
      where: { channelExpiresAt: { not: null } },
      orderBy: { channelExpiresAt: "asc" },
      select: { channelExpiresAt: true },
    }),
    prisma.sheetCreationSession.findFirst({
      where: { clanChoiceExpiresAt: { not: null } },
      orderBy: { clanChoiceExpiresAt: "asc" },
      select: { clanChoiceExpiresAt: true },
    }),
  ]);
  const deadlines = [nextChannel?.channelExpiresAt, nextChoice?.clanChoiceExpiresAt]
...
export async function startSheetScheduler(client: Client): Promise<void> {
  ...
  for (const session of stalled) {
    ...
    if (session.stage === "CLAN_CHOICE" && data.clanOptions.length) {
      await runClanAnimation(channel, session.id, data.clanOptions, session.clanChoiceExpiresAt).catch((error) =>

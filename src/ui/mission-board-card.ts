...
export interface MissionBoardCardData {
  ...
  offers: {
    ...
    locked: boolean;
    claimed: boolean;
    // Sem oferta pra esse rank: personagem ja concluiu todas as missoes
    // disponiveis. name/description vem null nesse caso.
    unavailable: boolean;
  }[];
}
...
export async function renderMissionBoardCard(data: MissionBoardCardData): Promise<Buffer> {
  ...
  const rows = data.offers.map((offer, index) => {
    ...
    const status = offer.unavailable ? "—" : offer.claimed ? "ACEITA" : offer.locked ? "RANK RESTRITO" : "DISPONÍVEL";
    const statusColor = offer.unavailable ? "#6b7280" : offer.claimed ? "#19804c" : offer.locked ? "#8d5b28" : "#456b91";
    const title = offer.unavailable ? "Sem missões disponíveis" : `[${offer.rank}] ${xml(offer.name ?? "")}`;
    const description = offer.unavailable
      ? [xml("Você já concluiu todas as missões deste rank."), ""]
      : wrap(offer.description ?? "", 72);
    return `<g opacity="${offer.locked || offer.unavailable ? ".62" : "1"}"><rect x="257" y="${y}" width="686" height="102" rx="13" fill="#fff4cf" fill-opacity=".7" stroke="#855721" stroke-opacity=".65"/><rect x="276" y="${y + 17}" width="67" height="50" rx="11" fill="${rankColor}" stroke="#fff0ac" stroke-width="2"/><text x="309" y="${y + 51}" text-anchor="middle" class="rank">${offer.rank}</text><text x="365" y="${y + 31}" class="mission">${title}</text><text x="365" y="${y + 54}" class="description">${description[0] ?? ""}</text><text x="365" y="${y + 72}" class="description">${description[1] ?? ""}</text><rect x="793" y="${y + 11}" width="128" height="26" rx="8" fill="${statusColor}" fill-opacity=".94"/><text x="857" y="${y + 29}" text-anchor="middle" class="status">${status}</text></g>`;
  }).join("");

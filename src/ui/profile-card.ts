...
export async function renderProfileCard(data: ProfileCardData): Promise<Buffer> {
  ...
  const portraitArtwork = image
    ? `<image href="${image}" x="60" y="80" width="365" height="500" preserveAspectRatio="xMidYMid slice" transform="translate(12 0)"/>`
    : artwork;
  ...
  const themedParchment = parchmentSvg
    ...
    // O selo entra por último: a linha dos elementos fica sempre atrás dele.
    .replace("</svg>", `${scrollRolls}${crossedKunai}</svg>`);

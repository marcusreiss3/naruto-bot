...
export async function renderAttributesCard(data: AttributesCardData): Promise<Buffer> {
  ...
  const rows = ATTRIBUTES.map((attribute, index) => {
    ...
    const inactive = attribute === "kugutsu" || attribute === "senjutsu";
    return `<g><rect x="${x}" y="${y}" width="475" height="48" rx="12" fill="#fff6d4" fill-opacity=".42" stroke="#a96c1b" stroke-opacity=".48"/><text x="${x + 18}" y="${y + 30}" class="attribute">${xml(ATTRIBUTE_LABELS[attribute])}</text>${inactive ? `<text x="${x + 220}" y="${y + 29}" class="muted">em desenvolvimento</text>` : ""}<rect x="${x + 395}" y="${y + 8}" width="62" height="32" rx="10" fill="#1d2740" stroke="#f3bd37" stroke-width="2"/><text x="${x + 426}" y="${y + 31}" text-anchor="middle" class="number">${total}</text>${added > 0 ? `<text x="${x + 370}" y="${y + 31}" text-anchor="end" class="pending">+${added}</text>` : ""}</g>`;
  }).join("");

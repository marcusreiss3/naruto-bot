...
describe("catálogo da Loja Premium", () => {
  ...
  it("busca produtos pelo identificador e rejeita identificadores desconhecidos", () => {
    ...
    expect(getPremiumProduct("attribute_respec")).toMatchObject({ kind: "RESPEC", cost: 1500 });
    expect(getPremiumProduct("fighting_style_reset")).toMatchObject({ kind: "FIGHTING_STYLE_RESET", cost: 1000 });
    expect(getPremiumProduct("character_reset")).toMatchObject({ kind: "CHARACTER_RESET", cost: 3000 });

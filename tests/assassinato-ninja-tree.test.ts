...
describe("árvore de Assassinato Ninja", () => {
  ...
  it("restringe toda a árvore a Kirigakure e a raiz também exige afinidade com Água", () => {
    const base: CharSnapshot = {
      charId: "c", name: "Teste", level: 50, spentByPool: {}, pointsByPool: { taijutsu: 50, ninjutsu: 50, kenjutsu: 50 },
      elements: ["AGUA"], owned: new Set(), clanId: null, attributes: { taijutsu: 50, ninjutsu: 50, kenjutsu: 50 }, villageId: "KONOHA",
    };
    // elemento ok, vila errada -> trava pela vila
  ...
  it("dá dano incondicional real, mais um reforço condicionado à preparação do assassinato", () => {
    ...
    expect(mods.damageMult).toBeCloseTo(1.08 * 1.10 * 1.08 * 1.07 * 1.12 * 1.06);
